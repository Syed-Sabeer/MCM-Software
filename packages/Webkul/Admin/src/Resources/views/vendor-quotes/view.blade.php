    ...
    <div class="flex flex-col gap-4">
        <div class="rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
            ...
            <div class="mt-4 grid gap-4 text-sm dark:text-white md:grid-cols-4">
                <div><strong>Vendor:</strong> {{ optional($vendorQuote->organization)->name }}</div>
                <div><strong>Job Order:</strong> {{ optional($vendorQuote->jobOrder)->job_order_number }}</div>
                <div><strong>Issue Date:</strong> {{ optional($vendorQuote->issue_date)->format('Y-m-d') }}</div>
                <div><strong>Expected Response:</strong> {{ optional($vendorQuote->expected_response_date)->format('Y-m-d') ?: '-' }}</div>
            </div>
        </div>
