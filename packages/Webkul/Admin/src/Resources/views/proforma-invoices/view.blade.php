                    ...
                    <tbody>
                        @forelse ($proformaInvoice->items as $item)
                            <tr class="border-b dark:border-gray-800">
                                ...
                                <td class="py-2 text-right">{{ $formatAmount($item->discount_amount ?: 0) }}</td>
                                <td class="py-2 text-right">{{ $formatAmount($item->tax_amount ?: 0) }}</td>
                                <td class="py-2 text-right font-medium">{{ $formatAmount($item->line_total ?: 0) }}</td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="10" class="py-4 text-center text-gray-500">No line items available.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div class="grid gap-4 md:grid-cols-2">
            <div class="rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900 dark:text-white">
                ...
                <div class="space-y-2 text-sm">
                    <div class="flex justify-between"><span>Sub Total</span><span>{{ $formatAmount($proformaInvoice->subtotal ?: 0) }}</span></div>
                    <div class="flex justify-between"><span>Discount</span><span>{{ $formatAmount($proformaInvoice->discount_amount ?: 0) }}</span></div>
                    <div class="flex justify-between"><span>Tax</span><span>{{ $formatAmount($proformaInvoice->tax_amount ?: 0) }}</span></div>
                    <div class="flex justify-between"><span>Advance Received</span><span>{{ $formatAmount($proformaInvoice->received_amount ?: 0) }}</span></div>
                    <div class="flex justify-between font-semibold"><span>Expected Balance</span><span>{{ $formatAmount($proformaInvoice->remaining_amount ?: 0) }}</span></div>
                    <div class="flex justify-between border-t pt-2 text-base font-bold"><span>Grand Total</span><span>{{ $formatAmount($proformaInvoice->grand_total ?: 0) }}</span></div>
                </div>
            </div>
