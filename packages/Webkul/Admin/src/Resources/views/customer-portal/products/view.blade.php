...
@section('content')
    ...
    <div class="grid gap-4 xl:grid-cols-[minmax(360px,0.9fr)_minmax(0,1.3fr)]">
        ...
        <section class="portal-card p-5">
            <div class="flex flex-wrap items-start justify-between gap-4 border-b border-gray-200 pb-4 dark:border-gray-800"><div><p class="portal-kicker">Product details</p><h2 class="mt-1 text-xl font-semibold text-gray-900 dark:text-white">{{ $product->name }}</h2><p class="mt-1 text-sm font-medium text-brandColor">Item Code: {{ $product->sku ?: '-' }}</p></div><span class="text-xl font-semibold text-gray-900 dark:text-white">{{ \Webkul\Admin\Http\Controllers\CustomerPortal\PortalController::money($product->selling_price ?: $product->price) }}</span></div>
