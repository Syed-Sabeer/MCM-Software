...
@section('content')
    ...
    <section class="portal-card overflow-hidden">
        ...
        <div class="divide-y divide-gray-200 md:hidden dark:divide-gray-800">
            @forelse($records as $contact)
                <article class="portal-row p-4"><div class="flex items-start gap-3"><span class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-brandColor font-semibold text-white">{{ strtoupper(substr($contact->name, 0, 1)) }}</span><div class="min-w-0 flex-1"><a href="{{ route('customer_portal.contacts.view', $contact->id) }}" class="font-semibold text-brandColor">{{ $contact->name }}</a><p class="mt-0.5 text-xs text-gray-500">{{ $contact->job_title ?: 'Company contact' }}</p>@if($contact->email)<a href="mailto:{{ $contact->email }}" class="mt-2 block truncate text-sm text-brandColor">{{ $contact->email }}</a>@endif @if($contact->phone || $contact->cell_phone)<a href="tel:{{ $contact->phone ?: $contact->cell_phone }}" class="mt-1 block text-sm text-gray-700 dark:text-gray-300">{{ $contact->phone ?: $contact->cell_phone }}</a>@endif</div><a href="{{ route('customer_portal.contacts.view', $contact->id) }}" aria-label="View {{ $contact->name }}"><span class="icon-right-arrow text-xl text-gray-400"></span></a></div></article>
            @empty
