...
@section('content')
    ...
    <section class="portal-card overflow-hidden">
                ...
                <tbody class="divide-y divide-gray-200 text-sm dark:divide-gray-800">
                    @forelse($records as $jobOrder)
                        <tr class="portal-row">
                            ...
                            <td class="px-4 py-3.5 text-gray-700 dark:text-gray-300">{{ \Webkul\Admin\Http\Controllers\CustomerPortal\PortalController::date($jobOrder->required_delivery_date, 'M d, Y', 'Not scheduled') }}</td>
        ...
        <div class="divide-y divide-gray-200 dark:divide-gray-800 md:hidden">
            @forelse($records as $jobOrder)
                <a href="{{ route('customer_portal.job_orders.view', $jobOrder->id) }}" class="portal-row block p-4"><div class="flex items-start justify-between gap-3"><div><p class="font-semibold text-brandColor">{{ $jobOrder->job_order_number }}</p><p class="mt-1 text-xs text-gray-500">{{ $jobOrder->subject ?: 'Customer order' }}</p></div>@include('admin::customer-portal.partials.status-badge', ['status' => $jobOrder->status])</div><div class="mt-4 grid grid-cols-2 gap-3 text-xs"><div><p class="text-gray-500">Delivery</p><p class="mt-1 font-medium dark:text-gray-200">{{ \Webkul\Admin\Http\Controllers\CustomerPortal\PortalController::date($jobOrder->required_delivery_date, 'M d, Y', 'Not scheduled') }}</p></div><div class="text-right"><p class="text-gray-500">Quantity</p><p class="mt-1 font-semibold dark:text-white">{{ \Webkul\Admin\Http\Controllers\CustomerPortal\PortalController::decimal($jobOrder->total_order_qty) }}</p></div></div></a>
            @empty
