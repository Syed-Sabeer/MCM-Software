<x-admin::layouts>
    ...
    @php
        ...
        $sourceEtd = $proformaInvoice->quote?->etd ?: $proformaInvoice->due_date;
        $formatInputDate = function ($value) {
            if (! $value || (int) $value->format('Y') <= 1) {
                return '';
            }
    ...
    <x-admin::form :action="route('admin.job_orders.store')" method="POST">
        <div class="flex flex-col gap-4">
            <div class="flex items-center justify-between rounded-lg border border-gray-200 bg-white px-4 py-3 dark:border-gray-800 dark:bg-gray-900">
                <div>
                    <div class="text-xl font-bold dark:text-white">{{ $existingJobOrder ? 'Update Job Order' : 'Create Job Order' }}</div>
                    <div class="text-sm text-gray-500">Source Proforma: {{ $proformaInvoice->proforma_number }}</div>
                </div>

                <button class="primary-button" data-loading-submit data-loading-text="{{ $existingJobOrder ? 'Updating...' : 'Creating...' }}">
                    {{ $existingJobOrder ? 'Update Job Order' : 'Create Job Order' }}
                </button>
            </div>
                    ...
                    <div class="grid gap-4 md:grid-cols-2">
                        ...
                        <div>
                            @include('admin::partials.document-status-picker', [
                                'type' => 'job_order',
                                'name' => 'status',
                                'selected' => old('status', $existingJobOrder?->status ?: 'open'),
                        ...
                        <div>
                            ...
                            <input type="date" name="issue_date" value="{{ old('issue_date', $existingJobOrder?->issue_date?->toDateString() ?: now()->toDateString()) }}" class="w-full rounded border border-gray-200 px-3 py-2 text-sm dark:border-gray-700 dark:bg-gray-800 dark:text-white">
                        </div>

                        <div>
                            <label class="mb-1 block text-sm font-medium dark:text-white">ETD</label>
                            <input type="date" name="required_delivery_date" value="{{ old('required_delivery_date', $formatInputDate($sourceEtd)) }}" class="w-full rounded border border-gray-200 px-3 py-2 text-sm dark:border-gray-700 dark:bg-gray-800 dark:text-white">
                        </div>
                    </div>
